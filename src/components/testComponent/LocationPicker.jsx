...
const DriverMapInOnline = () => {
    ...
    return (
        <div className="relative w-full h-screen">
            ...
            <MapContainer center={[60.7076, 28.7528]} zoom={13} className="absolute inset-0">
                <TileLayer
            ...
            <div className="absolute bottom-0 left-0 w-full bg-white bg-opacity-90 p-4 shadow-lg">
                <div className="location-status mb-2 p-2 border border-gray-300 rounded">
                ...
                {isOnline ? (
                    <>
                        {loading ? (
                            <p className="loading-message text-blue-500">Загрузка активных заказов...</p>
                        ) : (
                            <>
                                <h3 className="font-bold mb-2">Активные заказы</h3>
                                <OrderList orders={activeOrders} onSelectOrder={handleSelectOrder} selectedOrderId={selectedOrder?.id} />
                            </>
                        )}
                    </>
                ) : (
                    <p className="text-gray-500">Чтобы получать заказы, выйдите на линию</p>
                )}

                <div className="telegram-info mt-4 text-sm text-gray-600">
                    <strong>Ваш ID в Telegram:</strong> {userId}
                </div>
            </div>
        </div>
    );
